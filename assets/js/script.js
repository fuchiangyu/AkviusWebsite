...
const normalizeVisibleText = (root = document.body) => {
  if (!root || typeof String.prototype.normalize !== "function") return;
...
const getLocalizedVersionNotes = (version) => {
  const key = String(version?.key || "").replace(/-/g, "").toUpperCase();
  const notes = version?.notes || [];
  return notes.map((note, index) => {
    const translationKey = key ? `log${key}Note${index + 1}` : "";
    const translated = translationKey ? t(translationKey) : "";
    return translated && translated !== translationKey ? translated : note;
  });
};
...
const applyLanguage = () => {
  ...
  document.querySelectorAll("[data-version-note]").forEach((node) => {
    ...
    if (node.matches("ul")) {
      node.innerHTML = "";
      getLocalizedVersionNotes(version).forEach((note) => {
